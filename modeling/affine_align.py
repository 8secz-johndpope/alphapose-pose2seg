...
def affine_align_gpu(features, idxs, align_size, Hs):
    ...
    flow = F.affine_grid(theta=Hs_var, align_corners=True, size=(N, C_feat, H_feat, W_feat)).float().cuda()
    ...
    rois = F.grid_sample(feature_select, flow, mode='bilinear', align_corners=True,padding_mode='zeros') # 'zeros' | 'border' 
    return rois
